        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((hg) => (
              <TableRow key={hg.id} className="bg-slate-50 hover:bg-slate-50 border-slate-200">
          ...
          <TableBody>
            {isLoading ? (
              // Skeleton Loading State — 8 sahte satır
      ...
      <div className="flex items-center justify-between px-1">
        <div className="text-xs text-slate-500">
          {isLoading ? (
            ...
            <span>
              Toplam <span className="font-semibold text-slate-700">{table.getFilteredRowModel().rows.length}</span> sipariş —{" "}
              Sayfa <span className="font-semibold text-slate-700">{table.getState().pagination.pageIndex + 1}</span> / <span className="font-semibold text-slate-700">{table.getPageCount()}</span>
            </span>
          )}
        </div>
